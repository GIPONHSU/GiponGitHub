...
export function drawEntities(ctx: CanvasRenderingContext2D, engine: GameEngine, sprites: Record<string, HTMLCanvasElement>) {
        ...
        engine.zombies.forEach(z => {
            ...
            if (z.type === 'zombie_boss' && !(z as any).isDying) {
                ...
                if (isInvincibleOrSpecial) {
                    ...
                    if ((z as any).weakCornerIndex !== undefined && bossState === 'warning') {
                        ctx.save();
                        const currentShieldRad = 202.5 + 7.5 * Math.sin(Date.now() / 90);
                        const headingAngle = z.angle - Math.PI / 2;
                        const weakAngle = headingAngle + ((z as any).weakCornerIndex * Math.PI) / 4;
    ...
    ctx.globalAlpha = 0.6;
            ...
            if ((z.type === 'zombie_big' || z.type === 'zombie_bomb' || z.type === 'zombie_bouncing' || z.type === 'zombie_golden') && !(z as any).isDying) {
                ...
                // 3. Draw Ticket Text
                let ticketText = '20';
                if (z.type === 'zombie_big') ticketText = '6';
                else if (z.type === 'zombie_bomb') ticketText = '8';
                else if (z.type === 'zombie_bouncing') ticketText = '10';
                else if (z.type === 'zombie_golden') ticketText = '15';
        ...
        engine.tops.forEach((top) => {
            ...
            if ((top.state === 'standby' || top.launchPadState === 'charging' || top.launchPadState === 'flying') && top.hp > 0) {
                ctx.save();
                const time = Date.now();
            ...
            const isSuperOrLaunch = (top.superTimer !== undefined && top.superTimer > 0) || (top.rainbowSuperTimer !== undefined && top.rainbowSuperTimer > 0) || top.launchPadState !== undefined;
            if (isSuperOrLaunch) {
                ctx.save();
            ...
            if ((top.superTimer !== undefined && top.superTimer > 0) || (top.rainbowSuperTimer !== undefined && top.rainbowSuperTimer > 0) || top.launchPadState !== undefined) {
                ctx.save();
                ctx.lineWidth = 6;
            ...
            const needsFastSpin = !top.isAI && (
                                  (top.struggleMashCount !== undefined) || 
                                  (engine.zombieSiegeActive && engine.siegeStatus === 'clinging' && engine.siegeTargetPlayerId === top.id) ||
                                  (top.spinTutorialTimer !== undefined && top.spinTutorialTimer > 0) ||
                                  (top.coopState !== undefined && (top.coopState.phase === 'standoff' || top.coopState.phase === 'retreat_rotate')) ||
