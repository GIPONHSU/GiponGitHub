...
function getSelectionSprite(color: string, pilotColor: string, modelType: number): HTMLCanvasElement {
...
export function drawTutorialBubbleAtUI(
    ...
    const barW = 460;
    const positions = [
        { x: padding, y: CANVAS_H - 109 },
        { x: CANVAS_W - barW - padding, y: CANVAS_H - 109 },
        { x: padding, y: padding },
    ...
    let drawX = pos.x;
    let drawY = pos.y;
    if (top && top.damageShockTimer !== undefined && top.damageShockTimer > 0 && top.hp > 0) {
...
export function drawUIWorld(ctx: CanvasRenderingContext2D, engine: GameEngine) {
        ...
        if (bossStruggle) {
            ...
            if (player) {
                ...
                // Draw clashing "快轉動陀螺!!" overhead or underhead text in world-space
                if (GameUtils.getLatestActiveTutorial(engine, player) === 'boss_struggle') {
                    ...
                    drawTutorialBubble(ctx, midX, midY, currentR, engine.activeArenaCenterY ?? 540, "快轉動陀螺!!", picTopuse2Image, isPlayerUpsideDown);
                }
            }
        }
...
function drawModelSelectionUI(
    ...
    ctx.fillText(`${prof.label} 請選擇陀螺型號 (按鈕確認)`, drawX + barW / 2, drawY + 15);
    ...
    for (let i = 0; i < numModels; i++) {
        ...
        const optionY = drawY + 45;
        const isSelected = pStat.selectedModelIndex === i;

        // Draw background
        ctx.fillStyle = isSelected ? 'rgba(56, 189, 248, 0.2)' : 'rgba(30, 41, 59, 0.5)';
        ...
        if (sprite) {
            ctx.drawImage(sprite, optionX + 5, optionY + 5, optionWidth - 10, optionWidth - 10);
        }
...
export function drawUI(ctx: CanvasRenderingContext2D, engine: GameEngine) {
    ...
    const positions = [
        { x: padding, y: CANVAS_H - barH - padding, align: 'left' },
        { x: CANVAS_W - barW - padding, y: CANVAS_H - barH - padding, align: 'right' },
    ...
    PLAYER_PROFILES.forEach((prof, idx) => {
        ...
        let drawX = pos.x;
        let drawY = pos.y;
        if (isAlive && top && top.damageShockTimer !== undefined && top.damageShockTimer > 0 && top.hp > 0) {
            const shakeIntensity = 8 * (top.damageShockTimer / 0.45);
            drawX += (Math.random() - 0.5) * shakeIntensity;
            drawY += (Math.random() - 0.5) * shakeIntensity;
        }
        ...
        const drawTextWithShadow = (text: string, x: number, y: number, color: string, font: string) => {
            ctx.save();
            ctx.font = font;
            ctx.textAlign = 'left';
            ctx.textBaseline = 'middle';
        ...
        drawTextWithShadow(labelText, drawX + 10, drawY + 19, prof.color, 'bold 24px "Courier New"');
        ...
        drawTextWithShadow(` | 彩票: ${score}`, drawX + 10 + labelW, drawY + 19, '#ffffff', 'bold 20px "Courier New"');

        // Line 2: Coin Prompt / Token Count (moved up from Line 3)
        const coinText = `按[${idx + 1}]投幣 | 代幣:${top ? (top.coins ?? 0) : 0}`;
        drawTextWithShadow(coinText, drawX + 10, drawY + 57, '#10b981', 'bold 16px "Courier New"');
        ...
        if (isAlive && top) {
            ...
            if (((top.superTimer !== undefined && top.superTimer > 0) || (top.rainbowSuperTimer !== undefined && top.rainbowSuperTimer > 0)) && isCastingSkill) {
                activeCount = 10;
            }
            const isBarFullSpin = top.spin >= (top.maxSpin || 1000) || (((top.superTimer !== undefined && top.superTimer > 0) || (top.rainbowSuperTimer !== undefined && top.rainbowSuperTimer > 0)) && isCastingSkill);
        ...
        if (isAlive && top && top.missions) {
            ...
            top.missions.forEach((mission, mIdx) => {
                ...
                if (mission.completed) {
                    ...
                    ctx.fillStyle = `rgba(250, 204, 21, ${0.2 + 0.2 * Math.sin(Date.now() / 150)})`;
                ...
                let iconText = '';
                if (mission.targetType === 'zombie_small') { iconColor = '#22c55e'; iconText = '小'; }
                else if (mission.targetType === 'zombie_big') { iconColor = '#9333ea'; iconText = '紫'; }
                else if (mission.targetType === 'zombie_bomb') { iconColor = '#8b4513'; iconText = '咖'; }
                else if (mission.targetType === 'zombie_bouncing') { iconColor = '#be185d'; iconText = '粉'; }
                ...
                ctx.fillText(`${mission.currentCount}/${mission.targetCount}`, sX + 85, sY + slotH / 2);
        ...
        if (isAlive && top && top.hp <= 0) {
            ...
            const msg = timeLeft > 0 ? `快投幣補充能量!! (${timeLeft})` : "快投幣補充能量!!";
