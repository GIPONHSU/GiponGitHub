...
import { checkCircleBoxCollision, resolveCircleBoxCollision } from './physics';
    ...
    export function updateTopStandby(top: Top, engine: GameEngine, dt: number) {
        ...
        let currentOrbitSpeed = isBreakout ? 15.0 : (standbyOrbitSpeed + (spinningOrbitSpeed - standbyOrbitSpeed) * (top.spinInputFactor ?? 0));
        
        if ((top as any).isRevivedIdle) {
            currentOrbitSpeed = 0;
            let hasInput = top.isAI;
            if (!hasInput && top.controls) {
                hasInput = engine.keys.has(top.controls.up) ||
                           engine.keys.has(top.controls.down) ||
                           engine.keys.has(top.controls.left) ||
                           engine.keys.has(top.controls.right) ||
                           engine.keys.has(top.controls.spin) ||
                           engine.keys.has(top.controls.skill) ||
                           engine.keys.has('Digit7') || engine.keys.has('Numpad7');
            }
            if (hasInput) {
                (top as any).isRevivedIdle = false;
                top.standbyCenterVx = 0;
                top.standbyCenterVy = 0;
