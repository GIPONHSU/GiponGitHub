...
export function handleKeyDown(engine: GameEngine, e: KeyboardEvent) {
    ...
    if (!engine.keys.has(e.code)) {
        ...
        // Model Selection UI Input Handling
        for (let i = 0; i < 4; i++) {
            const p = engine.participants.find(part => part.id === `top_${i}`);
            if (p && p.isActive && p.selectionState === 'selecting') {
                const controls = PLAYER_PROFILES[i].controls;
                // Check direction keys for switching selection
                if (e.code === controls.left || e.key === controls.left) {
                    p.selectedModelIndex = ((p.selectedModelIndex || 0) + 3) % 4; // Previous
                    SoundSystem.play('pickupCoin_1'); // Optional UI blip
                } else if (e.code === controls.right || e.key === controls.right) {
                    p.selectedModelIndex = ((p.selectedModelIndex || 0) + 1) % 4; // Next
                    SoundSystem.play('pickupCoin_1');
                }
                
                // Check skill key for confirm
                if (checkSkillPress(e, controls)) {
                    // Confirm selection
                    p.modelType = (p.selectedModelIndex || 0) + 1;
                    p.selectionState = 'selected';
                    SoundSystem.play('pickupCoin_1'); // confirmation sound
                    
                    // Spawn the top
                    engine.spawnTopForParticipant(i);
                    const top = engine.tops.find(t => t.id === `top_${i}`);
                    if (top) {
                        EffectSystem.addParticles(engine, top.x, top.y, '#10b981', 12, 180, 5);
                        EffectSystem.addParticles(engine, top.x, top.y, '#34d399', 8, 240, 4);
                    }
        ...
        engine.tops.forEach(top => {
            ...
            if (!top.isAI && top.controls && top.hp > 0) {
                ...
                if (isDirPress && !isClinging && (top.spin >= 0 || (top.superTimer !== undefined && top.superTimer > 0) || (top.rainbowSuperTimer !== undefined && top.rainbowSuperTimer > 0))) {
