...
export function applyDamageToZombie(engine: GameEngine, z: import('../types').Zombie, damage: number, sourceTopId: string) {
    ...
    if (z.type === 'zombie_boss') {
        if (Math.random() < 1/1000 || currentHits >= 4000) {
            instakill = true;
        }
...
const MISSION_CONFIG = {
    zombie_small: [25, 50, 100],
    zombie_big: [5, 15, 25],
    zombie_bomb: [5, 10, 15],
    zombie_bouncing: [5, 10, 15]
};

function addRandomMission(top: Top) {
    const activeTypes = top.missions!.map(m => m.targetType);
    const availableTypes = ['zombie_small', 'zombie_big', 'zombie_bomb', 'zombie_bouncing'].filter(
        t => !activeTypes.includes(t as any)
    );
    if (availableTypes.length === 0) return;
    
    const validTypes = availableTypes.filter(t => top.missionTiers![t as keyof typeof top.missionTiers] < MISSION_CONFIG[t as keyof typeof MISSION_CONFIG].length);
    if (validTypes.length === 0) return;
    
    const chosenType = validTypes[Math.floor(Math.random() * validTypes.length)] as keyof typeof MISSION_CONFIG;
    const tier = top.missionTiers![chosenType];
...
export function handleZombieKillForMissions(engine: GameEngine, topId: string, zombieType: string) {
    ...
    for (const mission of top.missions) {
    ...
    if (freshlyCompleted > 0) {
        const postCompleted = top.missions.filter(m => m.completed).length;
        if (postCompleted >= 2) {
            const previousCompleted = postCompleted - freshlyCompleted;
            if (previousCompleted < 2) {
                // Trigger power up once we reach 2 completions
                top.superTimer = 30.0;
                
                engine.levelUpSlowMoTimer = 1.0;
                engine.levelUpTargetTopId = top.id;
                ...
                engine.floatingTexts.push({
                    ...
                    y: top.y - 50,
                    text: '陀螺強化!!',
                    color: '#facc15', // yellow-ish golden
                    life: 2.0,
                    maxLife: 2.0,
                    vy: -40,
                    scale: 2.0
...
export function tickMissions(top: Top, dt: number) {
    if (!top.missions) return;
    
    let needsReplacement = false;
    for (const mission of top.missions) {
        if (mission.completed && mission.glowTimer !== undefined && mission.glowTimer > 0) {
    ...
    if (needsReplacement) {
        const postCompleted = top.missions.filter(m => m.completed).length;
        if (postCompleted >= 2) {
            // Only replace if we have at least 2 completed
...
export function handleZombieDeath(engine: GameEngine, z: import('../types').Zombie, killerId: string): boolean {
    ...
    if ((z.type === 'zombie_big' || z.type === 'zombie_bomb' || z.type === 'zombie_bouncing' || z.type === 'zombie_golden') && !(z as any).isDying) {
    ...
    engine.zombies.forEach(z => {
        ...
        if (dist <= range + zRadius) {
            ...
            if (cd <= 0) {
                ...
                // Deduct 1 energy point per collision during launch/sweep, but per user request, NO HP deduction during rainbow power-up state!
                if (!(top.rainbowSuperTimer !== undefined && top.rainbowSuperTimer > 0)) {
                    top.hp = Math.max(1, top.hp - 1);
