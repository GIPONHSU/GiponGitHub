...
export function spawnBoss(engine: GameEngine) {
    ...
    let realHp = 53; // Increased by 50% from 35
    ...
    const bossObj: Zombie = {
        ...
        bossAttackIndex: 0,
        maxHp: 53, // Increased by 50% from 35
        hp: 53, // Increased by 50% from 35
...
export function spawnZombie(engine: GameEngine) {
    ...
    let isBig = Math.random() < 0.3; // 30% big (15% purple, 10% brown, 5% pink)
    ...
    if (isBig) {
        ...
        // Out of the 30% big zombies:
        // Pink (bouncing): 5/30 = ~16.67%
        // Brown (bomb): 10/30 = ~33.33%
        // Purple (big): 15/30 = 50%
        if (rand < (5 / 30)) {
    ...
    engine.zombies.push({
        ...
        mass: isBig ? 30 : 8,
        markForDeletion: false,
        hp: spawnType === 'zombie_bomb' ? 4 : (isBig ? 4 : 2),
        maxHp: spawnType === 'zombie_bomb' ? 4 : (isBig ? 4 : 2),
        angle: 0,
...
export function spawnGoldenZombie(engine: GameEngine) {
    // Check if golden zombie already exists
    if (engine.zombies.some(z => z.type === 'zombie_golden' && !z.markForDeletion)) {
        return; // Only 1 golden zombie at a time
    }

    const r = 64; // Same as purple zombie
    const R = 480;
    const leftCenterX = 540;
    const rightCenterX = 1380;
    const centerY = engine.activeArenaCenterY ?? 540;
    ...
    const dxP = 960 / Math.max(0.001, Math.abs(Math.cos(spawnAng)));
    const dyP = 540 / Math.max(0.001, Math.abs(Math.sin(spawnAng)));
    const dEdge = Math.min(dxP, dyP) + r + 50; 
    ...
    const walkTargetX = px + nx * (R + r + 20); 
    const walkTargetY = py + ny * (R + r + 20);
    const jumpTargetX = px + nx * (R - r - 60); 
    const jumpTargetY = py + ny * (R - r - 60);
    
    const goldenId = 'z_golden_' + Math.random();
    ...
    const goldenZombie = {
        ...
        vx: 0, vy: 0,
        radius: r,
        mass: 30, // Big zombie mass
        markForDeletion: false,
        hp: 30, // 30 hits to guarantee kill
        maxHp: 30,
        angle: 0,
        speedMultiplier: 0.5 + Math.random() * 1.0,
        hitCooldown: 0,
    ...
    engine.zombies.push(goldenZombie);
    
    // Spawn 10 orbiting green zombies
    for (let i = 0; i < 10; i++) {
        engine.zombies.push({
            id: 'z_orbit_' + Math.random(),
            type: 'zombie_small',
            x: sx, y: sy, // start at same position, updated by behavior
            vx: 0, vy: 0,
            radius: 24, // Small zombie radius
            mass: 8,
            markForDeletion: false,
            hp: 2,
            maxHp: 2,
            angle: 0,
            hitCooldown: 0,
            introState: 'done', // Orbiting zombies don't need intro, they just snap to golden
            orbitTargetId: goldenId,
            orbitAngleOffset: (Math.PI * 2 / 10) * i
        } as Zombie);
    }
}
