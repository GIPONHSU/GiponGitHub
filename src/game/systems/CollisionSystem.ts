...
export function handleCollision(engine: GameEngine, a: Entity, b: Entity) {
    ...
    if (res) {
        ...
        const aIsDeadTop = a.type === 'top' && (a as any).hp <= 0;
        const bIsDeadTop = b.type === 'top' && (b as any).hp <= 0;
        ...
        if (a.type === 'top' && b.type === 'top') {
            ...
            if (engine.gameMode === 'campaign') {
                ...
                if (targetToDamage && attacker) {
                    if ((targetToDamage as any).hp <= 0 || (attacker as any).hp <= 0) {
                    ...
                    } else if (targetToDamage.rainbowSuperTimer !== undefined && targetToDamage.rainbowSuperTimer > 0) {
            ...
            if (barrel.durability <= 0) {
                ...
                if (other.type === 'top') {
                    ...
                    const isInvulnerable = (topOther.rainbowSuperTimer !== undefined && topOther.rainbowSuperTimer > 0) || (topOther.breakoutOrbitTimer !== undefined && topOther.breakoutOrbitTimer > 0) || (topOther as any).hp <= 0;
                    if (isInvulnerable) {
...
export function hitZombie(engine: GameEngine, top: Top, z: Zombie, impact: number) {
        ...
        if (!isSuper) {
            if (top.hitCooldown === undefined || top.hitCooldown <= 0) {
                ...
                if (top.state === 'standby' && !top.isSpinning) {
                    ...
                    top.deflectionVx = (top.deflectionVx || 0) + dirVx * 150 * spinMultiplier * 1.125;
                    top.deflectionVy = (top.deflectionVy || 0) + dirVy * 150 * spinMultiplier * 1.125;
                    top.bounceTimer = 0.3;
    ...
    if (z.type === 'zombie_boss') {
        ...
        if (boss.bossAttackState === 'warning' || boss.bossAttackState === 'dash' || boss.bossAttackState === 'earthquake_leap' || (boss.introZ !== undefined && boss.introZ > 0)) {
            ...
            if (boss.bossAttackState === 'warning' && boss.weakCornerIndex !== undefined) {
                const shieldRad = 202.5 + 7.5 * Math.sin(Date.now() / 90);
                const headingAngle = z.angle - Math.PI / 2;
                const weakAngle = headingAngle + (boss.weakCornerIndex * Math.PI) / 4;
                const weakX = z.x + shieldRad * Math.cos(weakAngle);
                const weakY = z.y + shieldRad * Math.sin(weakAngle);
                
                const distToWeak = Math.hypot(top.x - weakX, top.y - weakY);
                if (distToWeak <= top.radius + 118.125) { // 157.5px decreased by 25% = 118.125px hit radius
                    if (Math.random() < 0.25) { // 1/4 chance to break
                        boss.bossAttackState = 'idle';
                        boss.bossNextAttackTime = 3.0;
                        boss.weakCornerIndex = undefined;
                        
                        SoundSystem.play('Attack_Break_012');
                        SoundSystem.play('SE-Explo1');
                        
                        if (Math.random() < 1/1000) {
                            z.hp = 0; // Trigger boss death sequence via its hp setter
                        }
                        ...
                        // Triangle shatter particles (Red & Yellow)
                        for (let i = 0; i < 8; i++) {
                            engine.particles.push({
                                x: weakX,
                                y: weakY,
                        ...
                        // 2. Add radiating yellow star sparks
                        const sparkCount = 16;
                        for (let i = 0; i < sparkCount; i++) {
                            const baseAng = (i / sparkCount) * Math.PI * 2 + (Math.random() * 0.2 - 0.1);
                            const sparkSpeed = 300 + Math.random() * 400;
                            const colors = ['#facc15', '#fef08a', '#eab308'];
            ...
            // Push the player top back (extremely strong during dash, moderate during warning setup) - Reduced by 25% per user request (multiplied by 1.125 instead of 1.5)
            const bounceForce = (isDash ? 1500 : 750) * 1.125;
            ...
            if (top.state === 'dash') {
                ...
                // Keep dynamic velocity consistent during dash deflection - Reduced by 25% (multiplied by 1.125 instead of 1.5)
                const maxDur = top.maxDashDuration ?? 1.0;
                const ratio = Math.max(0, Math.min(1, (top.dashTimer ?? 0) / maxDur));
                let dashSpeed = (400 + 1200 * Math.pow(ratio, 2.0)) * 1.125;
    ...
    if (z.type === 'zombie_big') {
        ...
        if (big.bigAttackState === 'warning' || big.bigAttackState === 'dash' || big.bigAttackState === 'earthquake_leap') {
            ...
            // Push the player top back - Reduced by 25% (multiplied by 1.125 instead of 1.5)
            const bounceForce = (isDash ? 1100 : 550) * 1.125;
    ...
    const isSuperHit = (top.rainbowSuperTimer !== undefined && top.rainbowSuperTimer > 0) || (top.breakoutOrbitTimer !== undefined && top.breakoutOrbitTimer > 0);
    const isBreakoutWhip = (top.breakoutOrbitTimer !== undefined && top.breakoutOrbitTimer > 0);
    ...
    // Reduce rebound process speed by 25% per user request (multiplied by 1.125 instead of 1.5)
    const finalDeflectPower = (baseDeflect + deflectScale) * lowSpinReboundBoost * 1.125;
    ...
    let offsetPower = (isDashing ? ((killsZombie || isHighSpin) ? 4 : 12) : 6) * 1.125; // Reduced by 25% (1.125 instead of 1.5)
    ...
    if (top.state === 'standby') {
        ...
        // Also scale up standby orbit center knockback when spin is low - Reduced by 25% (1.125 instead of 1.5)
        let kbPower = 580 * spinMultiplier * lowSpinReboundBoost * 1.125;
        if (isBoss && !top.isAI) {
            kbPower *= 2.0;
        }
        if (isHighSpin) {
    ...
    const isSpecialCharge = !!((top.state === 'dash') || (top.superTimer !== undefined && top.superTimer > 0) || (top.rainbowSuperTimer !== undefined && top.rainbowSuperTimer > 0) || (top.launchPadState === 'flying' || top.launchPadState === 'dashing') || isActivelyPushing);
    ...
    if (isBoss) {
        ...
        const isInterruptingDash = !!((top.state === 'dash') || (top.superTimer !== undefined && top.superTimer > 0) || (top.rainbowSuperTimer !== undefined && top.rainbowSuperTimer > 0) || (top.launchPadState === 'flying' || top.launchPadState === 'dashing'));
    ...
    const isBig = z.type === 'zombie_big' || z.type === 'zombie_bomb';
    ...
    if (z.hp <= 0) {
        ...
        if (match) {
            ...
            engine.spawnTicket(z.x, z.y, z.type, idx); // 擊倒殭屍掉落彩票
        }
