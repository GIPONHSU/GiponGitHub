...
export function createZombieGoldenSprite(): HTMLCanvasElement {
    const size = 48;
    const c = document.createElement('canvas');
    c.width = size * SCALE;
    c.height = size * SCALE;
    ...
    // Choose colors: golden
    const bodyColor = '#ca8a04'; // Dark golden
    const armColor = '#a16207'; // Deeper golden
    const headColor = '#facc15'; // Vibrant golden
    ...
    // Head
    drawPixelCircle(ctx, cx, cy - 6, 8, headColor);
    
    // Eyes
    const eyeSize = 2;
    const eyeOff = 3;
    drawPixelRect(ctx, cx - eyeOff, cy - 8, eyeSize, eyeSize, '#ef4444');
    drawPixelRect(ctx, cx + eyeOff - eyeSize, cy - 8, eyeSize, eyeSize, '#ef4444');
